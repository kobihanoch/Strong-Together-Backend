...
export const messages = messagesSchema.table(
  'messages',
  {
  ...
  (t) => [
    ...
    foreignKey({ name: 'messages_sender_id_fkey', columns: [t.senderId], foreignColumns: [users.id] })
      .onUpdate('cascade')
      .onDelete('cascade'),
    foreignKey({ name: 'messages_receiver_id_fkey', columns: [t.receiverId], foreignColumns: [users.id] })
      .onUpdate('cascade')
      .onDelete('cascade'),
...
export const messagesRelations = relations(messages, ({ one }) => ({
  ...
  receiver: one(users, { fields: [messages.receiverId], references: [users.id], relationName: 'messageReceiver' }),
}));
