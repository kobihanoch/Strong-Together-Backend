...
export function exerciseToSplitPolicies(t: { workoutsplitId: AnyPgColumn }) { const owns = drizzleSql`${uid} = (select wp."user_id" from "workout"."workoutplans" wp join "workout"."workoutsplits" ws on ws."workout_id" = wp."id" where ws."id" = ${t.workoutsplitId})`; const ownsForDelete = drizzleSql`exists (select 1 from "workout"."workoutsplits" ws join "workout"."workoutplans" wp on wp."id" = ws."workout_id" where ws."id" = ${t.workoutsplitId} and wp."user_id" = ${uid})`; return [
  ...
  pgPolicy('Enable delete for auth users on exercisetoworkoutsplit', { for: 'delete', to: authenticatedRole, using: ownsForDelete }),
]; }
