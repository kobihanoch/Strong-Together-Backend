...
export function exerciseToSplitPolicies(t: { workoutsplitId: AnyPgColumn }) { const owns = drizzleSql`${uid} = (select wp."user_id" from "workout"."workoutplans" wp join "workout"."workoutsplits" ws on ws."workout_id" = wp."id" where ws."id" = ${t.workoutsplitId})`; return [
