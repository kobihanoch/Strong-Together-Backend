...
export function workoutplansPolicies(t: { userId: AnyPgColumn }) { return [
  ...
  pgPolicy('Enable insert for auth users on workoutplans', { for: 'insert', to: authenticatedRole, withCheck: drizzleSql`${uid} = ${t.userId}` }),
  // Lets authenticated users update only workout plans they own.
  pgPolicy('Enable update for auth users on workoutplans', { for: 'update', to: authenticatedRole, using: drizzleSql`${uid} = ${t.userId}`, withCheck: drizzleSql`${uid} = ${t.userId}` }),
  // Lets authenticated users delete only workout plans they own.
  pgPolicy('Enable delete for auth users on workoutplans', { for: 'delete', to: authenticatedRole, using: drizzleSql`${uid} = ${t.userId}` }),
]; }
