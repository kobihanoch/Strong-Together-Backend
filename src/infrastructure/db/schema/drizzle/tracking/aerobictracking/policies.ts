...
export function aerobicTrackingPolicies(t: { userId: AnyPgColumn }) {
  return [
    ...
    pgPolicy('Enable delete for auth users on aerobictracking', {
      for: 'delete',
      to: authenticatedRole,
      using: drizzleSql`${uid} = ${t.userId}`,
    }),
