...
SET
  ...
  SELECT EXISTS (
    SELECT 1
    FROM social.crew_membership membership
    WHERE membership.crew_id = crew_id_in
...
SET
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (
        ORDER BY
          (membership.user_id = identity.current_user_id()) DESC,
          CASE membership.role WHEN 'leader' THEN 1 WHEN 'admin' THEN 2 ELSE 3 END,
          membership.joined_at,
          membership.id
      ) AS rank_order
    FROM social.crew_membership membership
    JOIN identity."user" u ON u.id = membership.user_id
    WHERE membership.crew_id = crew_id_in
      AND membership.status = 'active'
    ORDER BY rank_order
    LIMIT 5
...
"social"."get_top_crew_participants" (UUID),
