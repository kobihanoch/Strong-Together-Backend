...
export class SocialSummarySql {
  ...
  public get(): Promise<SocialSummarySqlRow[]> {
    return this.dbService.sql<SocialSummarySqlRow[]>`
      SELECT
        (
          SELECT COUNT(DISTINCT mine.crew_id)::INTEGER
          FROM social.crew_membership mine
          WHERE mine.user_id = identity.current_user_id ()
        ...
        COALESCE(
          (
            SELECT JSONB_AGG(
              ...
              ORDER BY preview.latest_joined_at DESC, preview.user_id
            )
            FROM (
              SELECT participant.user_id, MAX(participant.joined_at) AS latest_joined_at
              FROM social.crew_membership mine
              ...
              ORDER BY latest_joined_at DESC, participant.user_id
              LIMIT 3
            ) preview
