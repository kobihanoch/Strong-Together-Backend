...
export class MessagesSql {
  ...
  findByUser(userId: string, timezone: string): Promise<InboxMessageSqlRow[]> {
    return this.dbService.sql<InboxMessageSqlRow[]>`
      SELECT m.id, m.subject, m.msg, m.sent_at AT TIME ZONE ${timezone} AS "sentAt",
        m.is_read AS "isRead", u.name AS "senderFullName", u.profile_pic_path AS "senderProfilePicPath"
  ...
  markAsRead(messageId: string, userId: string): Promise<MessageMutationSqlRow[]> {
    return this.dbService.sql<MessageMutationSqlRow[]>`
      UPDATE messages.message SET is_read = TRUE
      WHERE id = ${messageId}::UUID AND receiver_id = ${userId}::UUID RETURNING id
    `;
  }
  ...
  deleteForUser(messageId: string, userId: string): Promise<MessageMutationSqlRow[]> {
    return this.dbService.sql<MessageMutationSqlRow[]>`
      ...
      WHERE id = ${messageId}::UUID AND (receiver_id = ${userId}::UUID OR sender_id = ${userId}::UUID) RETURNING id
    `;
  }
}
