...
import type { EmailChangeOutcome, UserOperationLogger } from '../models/update-user.models';
...
export class ConfirmEmailChangeUseCase {
   ...
   */
  async execute(token: string | undefined, logger: UserOperationLogger): Promise<EmailChangeOutcome> {
    ...
    try {
      ...
      if (error.code === '23505') {
        logger.warn({ event: 'user.email_change_conflict', userId: claims.sub }, 'Email already in use');
        return { statusCode: 409, reason: 'Email already in use' };
      }
      logger.error({ err: error, event: 'user.email_change_failed', userId: claims.sub }, 'Failed to update user email');
