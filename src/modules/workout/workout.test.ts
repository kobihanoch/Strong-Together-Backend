...
describe('Workouts', () => {
  ...
  it('rejects finishing a workout without workout_start_utc', async () => {
    ...
    const exercisetosplitId =
      addResponse.body.workoutPlan.workoutsplits
        ?.find((split: any) => split.name === 'A')
        ?.exercisetoworkoutsplit?.find((exercise: any) => exercise.exercise === 'Bench Press')?.id ??
      (await getExerciseToWorkoutSplitId(userId, 'A', 20));
