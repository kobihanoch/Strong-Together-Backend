...
export class WorkoutPlanQueries {
  ...
  async queryWholeUserWorkoutPlan(userId: string, tz: string): Promise<WholeUserWorkoutPlan[]> {
    return this.sql<WholeUserWorkoutPlan[]>`
      SELECT
        workoutplans.id::INT, workout.get_number_of_splits(workoutplans.id)::INT AS numberofsplits, workoutplans.created_at, workoutplans.user_id, workoutplans.is_active,
